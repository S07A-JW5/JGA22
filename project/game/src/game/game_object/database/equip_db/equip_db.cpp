...
void cEquipDataBase::Load()
{
	...
	for (int i = 1; i < EquipmentCSV.GetRows(); i++)
	{
		...
		Temp.Durability	= EquipmentCSV.GetInteger(i, 4);
		Temp.AttatchPart	= EquipmentCSV.GetInteger(i, 5);
		Temp.Weight		= EquipmentCSV.GetInteger(i, 6);
		Temp.Energy		= EquipmentCSV.GetInteger(i, 7);
		Temp.Heat			= EquipmentCSV.GetInteger(i, 8);
		Temp.Range		= EquipmentCSV.GetInteger(i, 9);
		Temp.DamageType	= (IUnit::DAMAGE_TYPE)EquipmentCSV.GetInteger(i, 10);
		Temp.MinDamage	= EquipmentCSV.GetInteger(i, 11);
		Temp.MaxDamage	= EquipmentCSV.GetInteger(i, 12);
