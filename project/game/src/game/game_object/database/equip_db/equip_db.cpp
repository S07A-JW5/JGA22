...
void cEquipDataBase::Load()
{
	...
	for (int i = 1; i < EquipmentCSV.GetRows(); i++)
	{
		...
		Temp.EffectID		= (uint8_t)EquipmentCSV.GetInteger(i, 13);
		Temp.Ammo			= (uint8_t)EquipmentCSV.GetInteger(i, 14);
		Temp.Resist[0]	= (int16_t)EquipmentCSV.GetInteger(i, 15);
		Temp.Resist[1]	= (int16_t)EquipmentCSV.GetInteger(i, 16);
		Temp.Resist[2]	= (int16_t)EquipmentCSV.GetInteger(i, 17);
		Temp.Protection	= (int16_t)EquipmentCSV.GetInteger(i, 18);
		Temp.Support		= (uint8_t)EquipmentCSV.GetInteger(i, 19);
		Temp.BaseHeat		= (int16_t)EquipmentCSV.GetInteger(i, 20);
		Temp.Inventory	= (uint8_t)EquipmentCSV.GetInteger(i, 21);
		Temp.MaxBatt		= (uint16_t)EquipmentCSV.GetInteger(i, 22);
		Temp.MaxAmmo		= (uint16_t)EquipmentCSV.GetInteger(i, 23);
		Temp.MaxPart		= (uint16_t)EquipmentCSV.GetInteger(i, 24);
		Temp.Power		= (uint8_t)EquipmentCSV.GetInteger(i, 25);
		Temp.Cooling		= (uint8_t)EquipmentCSV.GetInteger(i, 26);
